-- Migration 034: Remove account_goal_portions writes from RPC functions
-- Purpose: Make account_funds the single source of truth - update RPCs to use account_funds only
-- Created: 2025-01-30

-- Update spend_from_account_bucket to use account_funds only
...
BEGIN
  ...
  -- Sum goal funds for account (using account_funds instead of account_goal_portions)
  ...
  ELSIF v_type = 'goal' THEN
    ...
    -- Check if goal fund exists and has sufficient balance (using account_funds)
    IF NOT EXISTS (
      SELECT 1 FROM account_funds
      WHERE account_id = p_account_id AND type = 'goal' AND reference_id = v_id AND balance >= p_amount
    ) THEN
      RAISE EXCEPTION 'Insufficient goal funds in selected bucket';
    END IF;
    
    -- Update account_funds: Deduct goal fund balance
    UPDATE account_funds
    SET balance = GREATEST(balance - p_amount, 0),
        updated_at = NOW()
    WHERE account_id = p_account_id
...
-- Update receive_to_account_bucket to use account_funds only
...
BEGIN
  ...
  IF v_bucket_type_normalized = 'goal' AND p_bucket_id IS NOT NULL THEN
    -- Goal fund: Update account_funds only (account_goal_portions is now a view)
    -- Note: goal current_amount is updated separately by updateGoalProgress() function
