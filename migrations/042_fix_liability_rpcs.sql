-- Migration 042: Fix RPC functions to use account_funds instead of account_liability_portions
-- Purpose: Make RPC functions use account_funds as single source of truth for liability funds
-- Created: 2025-01-30

-- Update spend_from_account_bucket to use account_funds for liability calculations
-- This function was already updated for goals, now fixing liability portions
...
-- Update receive_to_account_bucket to use account_funds for liability funds
...
-- Verify RPC functions no longer reference account_liability_portions
DO $$
DECLARE
  v_func_name TEXT;
  v_func_def TEXT;
BEGIN
  -- Check for any remaining references to account_liability_portions in RPC functions
  SELECT routine_name, routine_definition INTO v_func_name, v_func_def
  FROM information_schema.routines
  WHERE routine_schema = 'public'
    AND routine_type = 'FUNCTION'
    AND routine_definition LIKE '%account_liability_portions%'
    AND routine_name NOT IN ('spend_from_account_bucket', 'receive_to_account_bucket')  -- These we just updated
  LIMIT 1;

  IF v_func_name IS NOT NULL THEN
    RAISE WARNING 'Function % still references account_liability_portions. Manual review needed.', v_func_name;
  ELSE
    RAISE NOTICE 'All RPC functions updated to use account_funds for liability funds';
  END IF;
END $$;
